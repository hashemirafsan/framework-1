...
use ArrayAccess;
use Glue\App\Exception\ExceptionHandler;
...
class Application implements ArrayAccess
{
	...
	protected function registerAppPaths()
	{
		...
		$this->bind('path.app', $path.'app/');
		$this->bind('path.config', $path.'config/');
		$this->bind('path.plugin', $path.'plugin/');
	...
	{
		if (@$this->getPluginSettings()['debug']) {
	...
	{
		return $this->settings['providers']['core'];
	}
	...
	{
		$bootstrappers = $this->settings['providers']['plugin'];
	 ...
	 * @param  string $alias
	 * @return string    
	 * @throws \Exception\UnResolveableEntityException    
	 */
